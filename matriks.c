...
/* Mengirim hasil perkalian matriks: salinan M1 * M2 */
{
	...
	MakeMATRIKS(NBrsEff(M1), NKolEff(M2), &ans);
   ...
   dan untuk setiap elemen M, M(i,j)=M(j,i) */
{
    ...
    if (IsBujurSangkar(M)) {
        for (i = GetFirstIdxBrs(M); i <= GetLastIdxBrs(M); i++) {
            for (j = GetFirstIdxKol(M); j <= GetLastIdxBrs(M); j++) {
   ...
   hanya maksimal 5% dari memori matriks yang efektif bukan bernilai 0 */
{
    ...
    while ((i<=GetLastIdxBrs(M))&&(count <= 0.05*NBElmt(M))) {
        j = GetFirstIdxKol(M);
        while ((j<=GetLastIdxKol(M))&&(count <= 0.05*NBElmt(M))) {
            if (Elmt(M,i,j) != 0) {
                count++;
            } else {
                j++;
...
/* Menghitung nilai determinan M */
{
	int i = 1;
	int j;
	int lkidx = GetLastIdxKol(M);
	int lbidx = GetLastIdxBrs(M);
	int ptemp = 1;
	int ntemp = 1;
	int pval = 0;
	int nval = 0;
	...
	else
	{
		while ( temp < lbidx )
		{
			...
			for (j = temp; j <= lkidx-1+temp; j++)
			{
				...
				//calculate element
				ptemp *= Elmt(M, tempi, tempj);
				ntemp *= Elmt(M, tempi, (lkidx+1-tempj));
				i++;
			}
			//holds value
			pval += ptemp;
